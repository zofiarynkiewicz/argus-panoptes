...
import { aiPluginPlugin } from './plugin';
...
describe('plugin', () => {
  it('should create and read TODO items', async () => {
    const { server } = await startTestBackend({
      features: [aiPluginPlugin],
    });

    await request(server).get('/api/ai-plugin/todos').expect(200, {
      items: [],
    });

    const createRes = await request(server)
      .post('/api/ai-plugin/todos')
      .send({ title: 'My Todo' });
    ...
    await request(server)
      .get('/api/ai-plugin/todos')
      .expect(200, {
        items: [createdTodoItem],
      });

    await request(server)
      .get(`/api/ai-plugin/todos/${createdTodoItem.id}`)
