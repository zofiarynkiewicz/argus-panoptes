...
const createMockCheckResult = (id: string, result: boolean) => ({
  check: {
    ...
    factIds: ['foundationPipelineStatusFactRetriever'],
  ...
  facts: {
    foundationPipelineStatusFactRetriever: {
      ...
      description: 'Foundation pipeline status facts',
      value: result ? 1 : 0,
    },
  },
  result,
...
describe('FoundationUtils', () => {
  let foundationUtils: FoundationUtils;

  beforeEach(() => {
    foundationUtils = new FoundationUtils();
    jest.clearAllMocks();
  });

  describe('getFoundationPipelineFacts', () => {
    it('should return parsed facts when data is available', async () => {
      ...
      const result = await foundationUtils.getFoundationPipelineFacts(
        mockTechInsightsApi,
        mockEntityRef,
      );
  ...
  describe('getFoundationPipelineChecks', () => {
    ...
    it('should return correct check results when success rate check is present and false', async () => {
      const mockCheckResults = [
        createMockCheckResult('foundation-success-rate', false),
      ];
